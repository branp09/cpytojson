import copy
from lib.copybook.parser import stmt
from lib.copybook.field_group import FieldGroup

def parse_file( filename:str, normalize_occurs=True, normalize_duplicate_field_names=True, calculate_positions=True) -> FieldGroup:
    result:FieldGroup = stmt.parseFile(filename,parseAll=True)
    # rearrange in a tree structure
    return _post_process(result,normalize_occurs=normalize_occurs,normalize_duplicate_field_names=normalize_duplicate_field_names,calculate_positions=calculate_positions)

def parse_string( contents:str, normalize_occurs=True, normalize_duplicate_field_names=True, calculate_positions=True ) -> FieldGroup:
...
def _post_process( result:FieldGroup, normalize_occurs:bool, normalize_duplicate_field_names:bool, calculate_positions:bool ) -> FieldGroup:
