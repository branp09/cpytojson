...
# bracket notation, e.g. X(2) | # picture notation, e.g. XX
char_pic = Group( (Suppress("X") + LPAR + Word(nums)("length") + RPAR) | OneOrMore("X").setParseAction(lambda tokens: len(tokens))("length"))("type_string")

numeric_pic = Group(
    Optional(
        Suppress("S")("signed")
        | (oneOf("+ -")+FollowedBy("9"))("signed")
        | (oneOf("+ -")+FollowedBy(".9"))("signed")
        | (oneOf("+ -")+FollowedBy("V9"))("signed")
    )
    + Optional(
